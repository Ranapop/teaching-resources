...
For choosing the car model, we don't need the switch, as a for is more compact:
```c
// Choose the car model
printf("Please choose the car model for brand %s\n",brands[brandChoice]);
for(int i=0;i<noOfBrands;i++) {
...
We can add the going back functionality in all cases (except the first one). After maintaining the state variable in each case and moving around some `getchar` calls (to do the clean up imeddiately after reading the input), we get to something like this:
```c
#include <stdio.h>
